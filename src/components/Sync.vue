...
export default {
    ...
    methods:{
        getClients(){
            ...
            axios.get(`https://preventa-dev.plantroya.com/api/v1/seller/poll/getclients?ruta=${this.ruta}`)
            .then(response => {
                if(response.data.status){
                    setTimeout(() => {
                        this.isFetching = false;
                        localStorage.setItem('clientes-entrega',JSON.stringify(response.data.rows))
                        this.isSet=true
                        this.ruta = ''
                    }, 500);
                    setTimeout(() => {
